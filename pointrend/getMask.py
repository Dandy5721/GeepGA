...
class fetalImage:
    ...
    def calBPD(self):
        ...
        if "BN-Skull-Out" in self.labels and "BN-Skull-In" in self.labels:
            ...
            print("skull-out or skull-in does not exist")
    #头围
    def calHC(self):
        if "BN-Skull-Out" in self.labels:
            ...
            print("skull-out does not exist")
    #腹围
    def calAC(self):
        if "PP-Abdomen-Axial-Planes" in self.labels:
            ...
            print("PP-Abdomen-Axial-Planes does not exist")
    #股骨
    def calFL(self):
    ...
    def calHL(self):
        if "LN-Humerus" in self.labels:
            ...
            print("LN-Humerus does not exist")
...
if __name__ == "__main__":
    ...
    args.input = ["C:\\Users\\aiyunji-xj\\Desktop\\newGT\\Canon_190916040955_@00000375@-05308-01727_[320-0-1600-900]_frame27519_Stop.jpg"]
    # args.input.append()
    print("haha",args.input)
    ...
    csvpath = "measurementResutl.csv"
    csvfile = open(csvpath, "w", encoding="utf-8", newline="")
    title = ["Name", "BPD", "HC", "AC", "FL", "PPC"]
    csv_writer = csv.writer(csvfile)
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            i=0
            for m in masks:
                ret, binary = cv2.threshold(m, 0, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\"+str(i)+".png",binary)

                print(binary.shape[1])
                print(binary.shape[0])

                small=cv2.resize(binary,dsize=(224,224))
                big=cv2.resize(small,(1600,900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                binary=binary[350:800,400:1200]
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\"+str(i)+"_224.png",binary)

                small = cv2.resize(binary, dsize=(112, 112))
                big = cv2.resize(small, (1600, 900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\" + str(i) + "_112.png", binary)

                small = cv2.resize(binary, dsize=(56, 56))
                big = cv2.resize(small, (1600, 900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\" + str(i) + "_56.png", binary)

                small = cv2.resize(binary, dsize=(28, 28))
                big = cv2.resize(small, (1600, 900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\" + str(i) + "_28.png", binary)

                small = cv2.resize(binary, dsize=(14, 14))
                big = cv2.resize(small, (1600, 900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\" + str(i) + "_14.png", binary)

                small = cv2.resize(binary, dsize=(7, 7))
                big = cv2.resize(small, (1600, 900))
                # cv2.imshow("mask",binary)
                # cv2.imshow("big",big)
                ret, binary = cv2.threshold(big, 127, 255, cv2.THRESH_BINARY)
                cv2.imwrite("C:\\Users\\aiyunji-xj\\Desktop\\figure\\" + str(i) + "_7.png", binary)
                # cv2.waitKey(0)
                i+=1
