...
if __name__ == "__main__":
    ...
    # 获取参数
    inputPath="C:\\Users\\aiyunji-xj\\detectron2\\projects\\PointRend\\input"
    filelist = os.listdir(inputPath)
    print(len(filelist))
    args = get_parser().parse_args()
    # "input\\朱玲玲23+3w_R.jpg"
    args.input = []
    # args.input = [inputPath+"\\8155508357_@000188441@.jpg"]

    for file in filelist:
        if file.endswith(".jpg"):
            args.input.append(os.path.join(inputPath, file))
    ...
    if args.input:
        ...
        # 下载进度
        print("*******")
        csvfile = open("Similarity.csv", "w", encoding="utf-8", newline="")

        title = ["Name", "BN-Skull-Out", 'BN-Skull-In',
                 'BN-CSP', 'BN-Cerebellar', 'BN-CM', 'Add-Ruler',
                 'BN-NF', 'BN-Lateral-Ventricle',
                 'PP-Abdomen-Axial-Planes', 'LN-Femur', 'LN-Humerus']
                ...
                if item in labels:
                    for shape in shapes:
                        ...
                        if label == item:
                            ...
                            ret, binary = cv2.threshold(masks[index], 0, 255, cv2.THRESH_BINARY)
                            contour_predict, hierarchy = cv2.findContours(binary, cv2.RETR_TREE,
                                                       cv2.CHAIN_APPROX_SIMPLE)
                            print(item)
                            contour_truth = np.array([[point] for point in points], dtype=np.int32)
                            # res=cv2.drawContours(img.copy(),contour_predict,-1,(0,255,0),2)#BGR
                            # res2 = cv2.drawContours(img.copy(), [contour_truth], -1, (0, 255, 0), 2)  # BGR
                            # cv2.imshow("predict",res)
                            # cv2.imshow("truth",res2)
                            # cv2.waitKey(0)
                            line.append(cv2.matchShapes(contour_predict[0], contour_truth, 3, 0.0))
                else:
                    line.append("")
            print(line)
